...
export const AIChat = () => {
  ...
  const handleSendMessage = async (attachment?: { mimeType: string; data: string }) => {
    ...
    try {
        const payload = attachment ? { message: input, image: attachment } : { message: input };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-100 dark:bg-black">
      <div className="p-4 pt-12 glass-panel shadow-lg z-10 flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="p-2 rounded-full hover:bg-white/10 text-slate-900 dark:text-white"><ArrowLeft /></button>
        <div>
